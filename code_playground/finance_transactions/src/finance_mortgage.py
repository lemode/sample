...
class MortgageService:
    ...
    def generate_schedule_of_values(self, period_date, rate_schedule):
        """
    ...
    def get_lifetime_interest_owing(self):
        ...
        number_of_periods = (
            constants.INITIAL_TERM * constants.COMPOUND_FREQUENCY_PER_ANNUM
    ...
    def get_monthly_payment(self):
        ...
        interest_rate = (
            (1 + annual_interest / compound_period)
    ...
    def generate_rates_schedule(
        ...
        while index < len(date_range):
            ...
            period_rate = self.generate_schedule_of_values(
                period_date, prime_rates_schedule
            )
            dict_prime_rates.update({period_date: period_rate})

            payment_due = self.generate_schedule_of_values(
                period_date, payment_due_schedule
            )
            dict_payment_due.update({period_date: payment_due})

            index += 1

        df_prime_rates = pd.DataFrame.from_dict(
            dict_prime_rates, orient="index"
        ).reset_index()
        ...
        df_prime_rates["interest_rate"] = (
            df_prime_rates["prime_rate"] + df_prime_rates["contract_rate"]
        )

        df_payment_due = pd.DataFrame.from_dict(
            dict_payment_due, orient="index"
        ).reset_index()
        ...
        df["interest_due"] = (
            (
                (1 + (df["interest_rate"] / 100) / compound_period)
                ** (compound_period / 12)
            )
            - 1
        ) * df["mortgage_balance"]
