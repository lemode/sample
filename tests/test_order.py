...
class TestOrderDataObject(TestCase):
    ...
    def test_groupby_sum_dataframe(self):
        ...
        payload_df= payload_df[(payload_df['Offer name'] == '60% Bonus promo') & (payload_df['Status'] == 'complete')]      

        #grab raw data and filter for records that should be summed
        raw_data_df = self.raw_data_df
        raw_data_df = raw_data_df[(raw_data_df['offerName'] == '60% Bonus promo') & (raw_data_df['status'] == 'complete')]
        
        result = self.sut.groupby_sum_dataframe(raw_data_df,self.data_constants.GROUPBY_LIST)

        #test that final result equals to final data set payload for a specific record
        self.assertEqual(payload_df['Total base points'].sum(), result['basePoints'].sum())
        self.assertEqual(payload_df['Total bonus points'].sum(), result['bonusPoints'].sum())
    ...
    def test_merge_dataframes(self):
        ...
        df2 = pd.DataFrame().from_dict([{'offerName':'50% Bonus promo','status':'pending','new_field': "4"}])
    ...
    def test_rename_dataframe_columns(self):
        ...
        #setup raw dataframe
        result = self.sut.rename_dataframe_columns(self.raw_data_df,self.data_constants.GROUPBY_COLUMNS)
